...
public class AddNewsTypePopup extends PopupWindow implements OnClickListener{
	...
	private int h;
	private int w;
	private FlowDeptLayout fl_type;
	...
	@SuppressLint("NewApi")
	public AddNewsTypePopup(MoreNewsActivity context,ArrayList<String> titles_id,
		...
		mChooseTypeView = inflater.inflate(R.layout.activity_explore_message, null);
		...
		fl_type = (FlowDeptLayout)mChooseTypeView.findViewById(R.id.fl_type);
...
//	        	tv_rk.append("\n\n"+tv_fck.getHeight()+","+tv_fck.getWidth()); 
	...
	protected void addType() {
		...
		for (String title : indicatorTitle) {
			DeptButton btn = new DeptButton(context);
			btn.setGravity(Gravity.CENTER);
			setStyle(btn);
			btn.setText(title);
			String type_id = title_id_copy.get(indicatorTitle_copy.indexOf(title));
			btn.setTag(type_id);
			fl_type.addView(btn);
		}
		setTextViewListner();
	...
	private void setTextViewListner() {
		// TODO Auto-generated method stub
		textViewCount = fl_type.getChildCount();
		Log.e("PopupWindow", "可删除标题数："+textViewCount);
		Button btn = (Button) fl_type.getChildAt(0);
		Log.e("PopupWindow", "第一个按钮标题："+btn.getText());
		
		for (i = 0; i < textViewCount; i++) {
			fl_type.getChildAt(i).setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					if(canRemove || textViewCount>1){
						if(fl_type.getChildCount()>1){		
							fl_type.removeView(v);
							Button btn = (Button) v;
							deleteElement(btn);
						}
					}
...
//	}
	...
	protected void deleteElement(Button btn) {
		// TODO Auto-generated method stub
		Log.e("PopupWindow", "删除标题："+btn.getText().toString());
		String title = btn.getText().toString();
		String type = btn.getTag().toString();
		title_id_copy.remove(type);
		...
		DHttpService.httpPost(paramMap, new AbstractDhomeRequestCallBack() {
			...
			public void success(JSONObject jsonObject) {
				...
				if (Constant.REQUESTCODE.equals(errorObj.getReturnCode()))
				{
					...
					ToastUtils.showMessage(context, "更新栏目成功");
				}else{
					Message msg = new Message();
					msg.what = Constant.HAVE_NEW_TYPE;
					Bundle bundle = new Bundle();
