...
public class MyCommentListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if (commentInfo_SecondLayer != null && !commentInfo_SecondLayer.equals("")) {
            //存在二级layer
            String puserid = userCard_SecondLayer.getUserSeqId();
            String currentuserid = Constant.userInfo.getUserSeqId();
            String targetstr = "回复" + userCard_SecondLayer.getUserName() + ":" + commentInfo_FirstLayer.getContent();
            int start = 2;
            int stop = start + userCard_SecondLayer.getUserName().length();
            CharSequence postContent = SpannableStringUtils.getClickableFaceSpan(context, targetstr, start, stop, puserid, currentuserid);
            holder.tv_commentcontent1.setText(postContent);
            // 设置该句使文本的超连接起作用
            holder.tv_commentcontent1.setMovementMethod(LinkMovementMethod.getInstance());

            if (commentInfo_SecondLayer.getpUserCard() == null || commentInfo_SecondLayer.getpUserCard().equals("")) {
                //直接评论
                String puserid1 = userCard_SecondLayer.getUserSeqId();
                String currentuserid1 = Constant.userInfo.getUserSeqId();
                String targetstr1 = userCard_SecondLayer.getUserName() + ":" + commentInfo_SecondLayer.getContent();
                int start1 = 0;
                int stop1 = start1 + userCard_SecondLayer.getUserName().length();
                CharSequence postContent1 = SpannableStringUtils.getClickableFaceSpan(context, targetstr1, start1, stop1, puserid1, currentuserid1);
                ...
                CharSequence postContent1 = SpannableStringUtils.getClickableFaceSpan(context, targetstr1, start1, stop1, start2, stop2, puserid1, puserid2, currentuserid1);
                holder.tv_commentcontent2.setText(postContent1);
            }
            // 设置该句使文本的超连接起作用
