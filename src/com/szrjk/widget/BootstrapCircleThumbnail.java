...
import com.szrjk.util.ImageUtils;
...
public class BootstrapCircleThumbnail extends FrameLayout
{
	...
	private void initialise(AttributeSet attrs)
	{
		...
		if (a.getString(R.styleable.BootstrapCircleThumbnail_bct_image) != null)
		{
			imageDrawable = a.getResourceId(
					R.styleable.BootstrapCircleThumbnail_bct_image, 0);
		...
		{
			this.size = a
					.getString(R.styleable.BootstrapCircleThumbnail_bct_size);
		}

		if (a.getString(R.styleable.BootstrapCircleThumbnail_bct_minimal) != null)
		{
			this.minimal = a.getBoolean(
					R.styleable.BootstrapCircleThumbnail_bct_minimal, false);
		}

		a.recycle();

		View v = inflator.inflate(R.layout.bootstrap_thumbnail_circle, null,
				false);
		...
		if (this.minimal == false)
		{
			...
			this.container.setPadding(paddingPX, paddingPX, paddingPX,
					paddingPX);
			container
					.setBackgroundResource(R.drawable.thumbnail_circle_container);
		}
		else
		{
			container
					.setBackgroundResource(R.drawable.thumbnail_circle_minimal);
		}
		...
		if (imageDrawable == 0)
		{
			...
			placeholder.setLayoutParams(new LinearLayout.LayoutParams(
					imageSizeWidthPX, imageSizeHeightPX));
	...
	public void setImage(int drawable)
	{
		...
		Bitmap roundBitmap = ImageUtils.getCircleBitmap(bitmap, widthPX,
				heightPX);
		image.setImageBitmap(roundBitmap);
